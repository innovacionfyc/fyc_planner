...
try {
    if (isset($cols['archived_at'])) {
        $up = $conn->prepare("UPDATE boards SET archived_at = NULL WHERE id=? LIMIT 1");
        $up->bind_param('i', $board_id);
        $up->execute();
        $_SESSION['flash'] = ['type' => 'ok', 'msg' => 'Tablero restaurado.'];
    } elseif (isset($cols['is_archived'])) {
        $up = $conn->prepare("UPDATE boards SET is_archived = 0 WHERE id=? LIMIT 1");
        $up->bind_param('i', $board_id);
        $up->execute();
        $_SESSION['flash'] = ['type' => 'ok', 'msg' => 'Tablero restaurado.'];
    } elseif (isset($cols['archived'])) {
        $up = $conn->prepare("UPDATE boards SET archived = 0 WHERE id=? LIMIT 1");
        $up->bind_param('i', $board_id);
        $up->execute();
        $_SESSION['flash'] = ['type' => 'ok', 'msg' => 'Tablero restaurado.'];
    } else {
